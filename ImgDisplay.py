...
class ScrollResizeDisplay(tk.Frame):
    ...
    def __init__(self, master=None, img=None, **kw):
        ...
        ###################
        self.cvs_frame = tk.Frame(self, width=self.canvas_w, height=self.canvas_h)
        self.cvs_frame.grid(row=2, column=1, sticky='n')
        #self.cvs_frame.bind("<Configure>", self.resize_displayed_image)  # TESTING
        ...
        self.v_scroller = tk.Scrollbar(self.cvs_frame, orient='vertical', width=24)
        self.v_scroller.grid(row=0, column=1, sticky='ns')
        self.h_scroller = tk.Scrollbar(self.cvs_frame, orient='horizontal', width=24)
...
if __name__ == '__main__':
    import Plat
    mp = Plat.MultiPlat.from_text('T154N-R97W Sec 14: NE/4', settings='letter')
    im = mp.output()[0]
    app = tk.Tk()
    disp = ScrollResizeDisplay(master=app, img=im)
    disp.pack()
    app.mainloop()